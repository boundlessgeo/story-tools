                    ...
                    if (style_href) {
                      ...
                      fetch(style_href).then(function(r) {
                        r.text().then(function(sldText) {
                          ...
                          var rule = sld_doc.getElementsByTagName(prefix + 'Rule')[0];

                          var prop;
                          var stroke_obj = getStrokeObject(prefix, rule);
                          for (prop in stroke_obj) {
                            $scope.activeStyle.stroke[prop] = stroke_obj[prop];
                          }

                          var fill_obj = getFillObject(prefix, rule);
                          for (prop in fill_obj) {
                            $scope.activeStyle.symbol[prop] = fill_obj[prop];
                          }

                          var label_obj = getLabelObject(prefix, rule);
